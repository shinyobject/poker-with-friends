...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", (payload: { roomId?: string; name?: string }, ack?: (res: unknown) => void) => {
  ...
  socket.on("start-hand", (_payload: unknown, ack?: (res: unknown) => void) => {
    if (!joinedRoom) {
  ...
  socket.on(
    "action",
    (
      payload: { action?: "fold" | "check" | "call" | "raise"; raiseAmount?: number },
...
function sanitizeRoomId(value: string | undefined): string {
  return (value ?? "")
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, "")
    .slice(0, 8);
